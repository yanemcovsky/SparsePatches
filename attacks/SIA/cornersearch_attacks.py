...
import numpy as np
import torch
from attacks.SIA.utils import get_logits, get_predictions
...
class CSattack():
  def __init__(self, model, args):
    ...
    self.eps = args['epsilon']           # for L0+Linf, the bound on the Linf-norm of the perturbation
    ...
    self.bs = self.shape_img[0]*self.shape_img[1]
  ...
  def report_schematics(self):
      print("Running CS attack based on the paper: \" Sparse and Imperceivable Adversarial Attacks \" ")
  ...
  def perturb(self, x_tensor, y_tensor, targeted=False):
    ...
    for c in range(x_nat.shape[0]):
      if corr_pred[c]:
        ...
        for counter in range(self.n_corners):
          #logit_2[counter*self.bs:(counter+1)*self.bs], pred = sess.run([self.model.y, self.model.correct_prediction], feed_dict={self.model.x_input: batch_x[counter*self.bs:(counter+1)*self.bs], self.model.y_input: np.tile(batch_y,(self.bs))})
          logit_2[counter*self.bs:(counter+1)*self.bs] = get_logits(self.model, batch_x[counter*self.bs:(counter+1)*self.bs])
          pred = logit_2[counter*self.bs:(counter+1)*self.bs].argmax(axis=-1) == np.tile(batch_y,(self.bs))
          if not pred.all() and not found:
            ind_adv = np.where(pred.astype(int)==0)
            adv[c] = batch_x[counter*self.bs + ind_adv[0][0]]
            found = True
            if self.verbose:
    ...
    adv_pert = adv - x_nat
    adv_pert[np.where(np.abs(adv_pert) <= 1e-10)] = 0

    if self.verbose:
      pixels_changed = np.sum(np.amax(np.abs(adv_pert) > 0, axis=-1), axis=(1, 2))
